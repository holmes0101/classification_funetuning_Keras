...
InResNetV2Dir = '/home/holmes/keras_model/inception_v3/inception_v3_weights_tf_dim_ordering_tf_kernels_notop.h5'
trainDataDir = currentDir + '/data/train'
valDataDir = currentDir + '/data/val'


base_model = InceptionV3(weights = InResNetV2Dir, include_top = False, pooling = 'avg')
...
modelDir = currentDir + '/data/model/InceptionV3/IncptionV3-ep{epoch:03d}-acc-{acc:.5f}-loss{loss:.5f}-val_acc:{val_acc:.5f}-val_loss{val_loss:.5f}.h5'
if not os.path.exists(currentDir + '/data/model/InceptionV3'):
    os.makedirs(currentDir + '/data/model/InceptionV3')
...
modelDir = currentDir + '/data/model/InceptionV3/InceptionV3-finetune_1024_last_epoch.h5'
model.save( modelDir )